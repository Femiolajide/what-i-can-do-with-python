...
st.title("CSV/Excel to SQL Table Generator`(PostgreSQL & MYSQL)`")
st.write("This tool converts CSV and Excel files into ready-to-use **PostgreSQL/MySQL** table creation scripts. It automatically formats column names into snake_case and derives the table name from the uploaded file (also standardised to snake_case).")
st.markdown("---")
...
def mysql_script(df,table_name:str) -> str:
    ...
    def mysql_create(df:pd.DataFrame,table_name:str):
        from pandas.io.sql import get_schema
        import re
        df = df.rename(columns=lambda x: clean_name(x))
        df = df.apply(lambda x: x.astype(str))
        script = get_schema(df,table_name).replace('"','`') + ";\n\n"
        script_2 = re.sub(r"^CREATE TABLE","INSERT INTO",script)
        script_2 = re.sub(r' [A-Z]+(?=,)| [A-Z]+(?=\n\);)',"",script_2).rstrip("\n;") + "\n\n"
        script_update = script.replace("CREATE TABLE",
                                       "CREATE TABLE IF NOT EXISTS")
        return script_update + script_2
    return mysql_create(df,table_name) + mysql_val(df)
...
if file:
    ...
    if not isinstance(data,str):
        ...
        if dbms == "PostgreSQL":
            ...
            data_table = mysql_script(data,table_name)
