...
with tab1:
   st.subheader("Start here to enter the data for your bar chart")
   var = st.text_input("Enter the variable name",
                    placeholder="Enter the variable name that \
represents the category being analysed (E.g. Gender, Departments...)")
   st.write("Enter your data category \
   and the corresponding frequency in the table below")
   # st.write("---")

   var = var if var else "Category"
   # dd = pd.DataFrame(columns=[var,"Frequency"])
   # st.data_editor(dd)

   data = pd.DataFrame(columns=[var,"Frequency"])
   data = st.data_editor(
      data=data,
      num_rows="dynamic",
      width="stretch",
      column_config={var:st.column_config.TextColumn(    
                     help="Enter your data **category** here!",
                     required=True
                  ),
                  "Frequency":st.column_config.NumberColumn(
                     help="Enter the **frequency** of the category",
                     min_value=0,
                     format="%d",
                     required=True
                  )})
...
if not data.empty and  not data[var].duplicated().any():
   with tab3:
      orient_type = st.radio("Select chart orientation",
         options=["Column Chart",
                  "Bar Chart"][::-1],
         captions=["Vertical bars (Best for small categories)",
                     "Horizontal bars (Great for many categories)"][::-1]
         )
      if orient_type == "Column Chart":
